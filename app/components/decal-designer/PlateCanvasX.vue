  ...
  <div class="panel plate-canvas-container">
    ...
    <div class="canvas-wrapper">
      <div class="flex flex-col items-center">
        <!-- 盤子背景 -->
        <div ref="plateContainerRef" class="plate-container relative shadow-lg" :style="{
          ...
          <div v-for="pattern in placedPatterns" :key="pattern.id" class="pattern-on-plate"
            ...
            }" @mousedown="startDragMove($event, pattern.id)">
            ...
            <div class="pattern-controls">
              ...
              <button class="rotate-handle" @mousedown.stop="startRotate($event, pattern.id)">
                <Icon name="ic:baseline-cached" class="text-blue-500" />
              </button>
...
const updatePatternAngle = (id: any, newAngleDeg: any) => {
  ...
  selectedPattern.value.angle = ((newAngleDeg % 360) + 360) % 360;
};
...
// 移动开始
const startDragMove = (event: any, id: string) => {
...
const startRotate = (event: any, id: string) => {
  designStore.selectPattern(id);
  // event.stopPropagation();
  // const pattern = patterns.value.find(p => p.id === id);
  // if (!pattern) return;
